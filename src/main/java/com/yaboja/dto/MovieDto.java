...
public class MovieDto implements Comparable<MovieDto>{
    ...
    @Override
    public int compareTo(MovieDto dto) {
    	int myPupdate = Integer.parseInt(this.pupdate.replace(".",""));
    	int inputPupdate = Integer.parseInt(dto.getPupdate().replace(".",""));
    	
        if (myPupdate < inputPupdate) {
            return -1;
        } else if (myPupdate > inputPupdate) {
            return 1;
        }
        return 0;
    }
